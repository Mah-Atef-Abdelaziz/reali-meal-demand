...
export default function Sidebar({ activeTab, setActiveTab, isBackendConnected, isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside className={`w-64 bg-charcoal-900 border-r border-charcoal-800 flex flex-col justify-between h-screen fixed md:sticky top-0 left-0 z-50 transition-transform duration-300 md:transform-none ${
        isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
      }`}>
        <div>
          {/* Brand Header */}
          <div className="p-6 flex items-center justify-between border-b border-charcoal-800 bg-charcoal-950/40">
            ...
            <button 
              onClick={onClose}
              className="p-1.5 rounded-lg bg-charcoal-800 text-charcoal-300 hover:text-white md:hidden cursor-pointer"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
